...
void USART5_DMA_Init(void) {
    ...
    HAL_UARTEx_ReceiveToIdle_DMA(&huart5, usart5_rx_buffer[usart5_rx_buffer_index], DBUS_RX_BUF_SIZE); // 接收完毕后重启
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef * huart, uint16_t Size)
{
    ...
    if (huart->Instance == USART2)
    {
        ...
        if (Size == sizeof(MotorData_t))
        {
            ...
            xQueueSendFromISR(motor_rx_queue, &msg, &xHigherPriorityTaskWoken);//已经把消息发送给队列了，不需要再进行任务处理
        }
        HAL_UARTEx_ReceiveToIdle_DMA(&huart2, usart2_rx_buffer, sizeof(usart2_rx_buffer));
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef * huart)
{
    if(huart->Instance == UART5)
    {
        HAL_UARTEx_ReceiveToIdle_DMA(&huart5, usart5_rx_buffer[usart5_rx_buffer_index], DBUS_RX_BUF_SIZE); // 接收发生错误后重启
